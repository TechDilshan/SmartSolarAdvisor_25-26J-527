...
def generate_prediction_pdf(prediction_id):
    ...
    elements.append(Paragraph("Solar Energy Prediction Report", title_style))
    ...
    elements.append(Paragraph(f"Monthly Report - {get_month_name(prediction.month)} {prediction.year}", subtitle_style))
    ...
    location_table = Table(location_data, colWidths=[4*cm, 5*cm])
    ...
    if prediction.monthly_savings_usd:
        ...
        financial_table = Table(financial_data, colWidths=[4*cm, 5*cm])
    ...
    elements.append(Paragraph("Smart Solar Advisor - IoT-Enabled Hybrid ML for Location-Aware Solar Prediction", footer_style))
...
def generate_annual_prediction_pdf(year):
    ...
    predictions = query.order_by(Prediction.month.asc()).all()
    ...
    elements.append(Paragraph(f"Year {year} - Complete Analysis", subtitle_style))
    elements.append(Paragraph(f"Generated on: {datetime.now().strftime('%B %d, %Y at %I:%M %p')}", subtitle_style))
    elements.append(Spacer(1, 0.5*cm))
    
    # Calculate totals
    total_annual_energy = sum(p.predicted_energy_kwh for p in predictions)
    avg_monthly_energy = total_annual_energy / 12
    ...
    avg_monthly_savings = total_annual_savings / 12
    ...
    summary_table = Table(summary_data, colWidths=[4.5*cm, 4.5*cm])
    summary_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#1976d2')),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
        ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
    ...
    location_data = [
        ...
        ['Average Shading Factor', f"{sum(p.shading_factor for p in predictions) / len(predictions):.3f}"]
    ]
    
    location_table = Table(location_data, colWidths=[4.5*cm, 4.5*cm])
    location_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#388e3c')),
        ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
        ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
    ...
    monthly_data = [['Month', 'Energy (kWh)', 'Savings (LKR)', 'Confidence']]
    ...
    monthly_table = Table(monthly_data, colWidths=[3*cm, 2.5*cm, 3*cm, 2*cm])
    ...
    if total_annual_savings > 0:
        ...
        financial_table = Table(financial_data, colWidths=[4.5*cm, 4.5*cm])
        financial_table.setStyle(TableStyle([
