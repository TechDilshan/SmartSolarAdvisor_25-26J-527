...
function LocationMarker({ position, onPositionChange }) {
  ...
  return <Marker position={markerPosition} draggable={true} 
    eventHandlers={{
      dragend: (e) => {
...
function LocationMap({ latitude, longitude, onLocationChange, height = '400px' }) {
  // Ensure proper coordinate order: [latitude, longitude]
  // Round to 4 decimal places for display
  const initialLat = latitude && !isNaN(latitude) && latitude >= -90 && latitude <= 90 
    ? Math.round(latitude * 10000) / 10000 : 7.2;
  const initialLon = longitude && !isNaN(longitude) && longitude >= -180 && longitude <= 180 
    ? Math.round(longitude * 10000) / 10000 : 80.0;
  ...
  const searchLocation = async (query) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&limit=5&countrycodes=lk&accept-language=en`,
        {
          headers: {
    ...
    <div className="location-map-container">
      <div className="map-controls">
        ...
        <div className="coordinate-inputs">
          <button type="button" className="geolocate-btn" onClick={handleGeolocate}>
            📍 Use My Location
          </button>
